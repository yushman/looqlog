...
The `live-tail-ui` capability covers the browser-side surfaces of a live stdin stream:
the connection-state indicator, the lines/sec counter, gap markers, autoscroll with
pause/resume, client-side retention/eviction, reconnect, stream format detection, and
the stdin-mode privacy indicator (`live-tail` change).
## Requirements
