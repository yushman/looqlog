...
The `cli` capability covers looq's argv surface: flag semantics, startup output, the
`--host` exposure warning, browser auto-open, and file-vs-stdin mode selection.
...
- **WHEN** the user runs `looq --colour`
...
- **WHEN** the user runs `looq does-not-exist.log`
...
- **WHEN** the user runs `looq --port 0 app.log`
