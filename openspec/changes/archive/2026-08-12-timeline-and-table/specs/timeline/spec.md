...
The timeline SHALL render a histogram of entry counts per time bucket over the dataset's time
span, using bucket widths drawn from a fixed ladder of human-readable intervals so that the axis
labels stay round at every zoom level.

#### Scenario: Counts match the source
- **WHEN** a fixture with known timestamps is opened
