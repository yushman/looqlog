...
//! `assets/` from `web/` sources and `crates/looqlog-wasm`; nothing here reads the
...
/// Fill in the `__LOOQLOG_HINT__`, `__LOOQLOG_MODE__` and `__LOOQLOG_TOKEN__` placeholders in
...
/// from untrusted input) that `looqlog-app.ts` reads to decide which UI to mount.
...
pub fn render_index(hint_html: &str, mode_json: &str, token: &str) -> String {
    INDEX_HTML_TEMPLATE
        .replace("__LOOQLOG_HINT__", hint_html)
        .replace("__LOOQLOG_MODE__", mode_json)
        .replace("__LOOQLOG_TOKEN__", token)
}
...
mod tests {
    ...
    fn render_index_substitutes_hint_and_mode() {
        ...
        assert!(!rendered.contains("__LOOQLOG_HINT__"));
        assert!(rendered.contains(r#"{"mode":"file"}"#));
        assert!(!rendered.contains("__LOOQLOG_MODE__"));
        assert!(rendered.contains("tok123"));
        assert!(!rendered.contains("__LOOQLOG_TOKEN__"));
    }
}
