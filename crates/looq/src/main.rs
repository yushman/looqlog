    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| tracing_subscriber::EnvFilter::new("looq=info")),
        )
        .init();
    ...
    let hint_html = match &path_hint {
        Some(path) => format!(
            "Open <code>{}</code> below — this is a hint from the command line; the \
             file is read by your browser, not by <code>looq</code> (see ADR-0007).",
...
fn print_banner(url: &str, mode: Mode, path_hint: Option<&str>, max_lines: usize) {
    println!("looq v{}", env!("CARGO_PKG_VERSION"));
    println!("\u{2192} {url}");
