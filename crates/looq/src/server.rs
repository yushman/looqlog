...
pub struct AppState {
    /// Pre-rendered HTML fragment for the `__LOOQ_HINT__` placeholder (already
    /// escaped; see `assets::html_escape`).
    pub hint_html: String,
    /// Pre-rendered JSON for the `__LOOQ_MODE__` placeholder (`{"mode":"file"}` or
    /// `{"mode":"stdin","maxLines":N}`), read by `looq-app.ts` to decide which UI to
