...
//!
//! These mirror `looqlog-core`'s types field-for-field but live entirely in this crate:
//! `looqlog-core` stays free of `serde::Serialize`/wasm concerns (ADR-0005), and the
...
pub struct DiagnosticsSummaryDto {
    ...
    /// Retained examples, capped at `cap` (see `looqlog_core::Diagnostics`).
