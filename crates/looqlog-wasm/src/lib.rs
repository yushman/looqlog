//! Thin `wasm-bindgen` adapter over `looqlog-core` (ADR-0005, `wasm-bridge` spec).
//!
//! `ParserHandle` wraps `looqlog_core::Parser` one-to-one: construct per file (design.md
...
//! `looqlog-core`; this crate only translates types across the JS boundary (`dto.rs`)
...
fn to_js<T: serde::Serialize + ?Sized>(value: &T) -> Result<JsValue, JsValue> {
    ...
    // (looqlog-filter-bar.ts's chip rendering), crashing the tab on any entry with no
    // level extracted.
    ...
    pub fn new(
        ...
        let tz = match tz_offset_minutes {
            Some(minutes) => {
                ...
                looqlog_core::TimeZonePolicy::fixed_offset(offset)
            }
            None => looqlog_core::TimeZonePolicy::utc(),
        };
        let mut ctx = looqlog_core::ParseContext::new(tz);
        ...
        Ok(ParserHandle {
            inner: looqlog_core::Parser::with_context(format, ctx),
        })
    }
