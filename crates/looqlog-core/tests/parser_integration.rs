...
use looqlog_core::diagnostics::DiagnosticReason;
use looqlog_core::entry::{Entry, FieldValue};
use looqlog_core::format::Format;
use looqlog_core::level::Level;
use looqlog_core::parser::{Parser, MAX_CHAIN_LINES};
use looqlog_core::timestamp::{ParseContext, TimeZonePolicy};
...
/// deterministic. `looqlog-core` never reads the clock (ADR-0005).
...
fn syslog_3164_fixture_is_dated_flagged_and_detected_as_a_match() {
    ...
    assert_eq!(
        ...
        Some(looqlog_core::TimestampShape::Syslog3164)
    );
...
fn klog_fixture_takes_its_level_from_the_severity_letter() {
    ...
    assert_eq!(
        detection.timestamp_shape,
        Some(looqlog_core::TimestampShape::Klog)
    );
    assert_eq!(detection.outcome, looqlog_core::DetectionOutcome::Threshold);
...
fn clf_fixture_is_found_past_offset_zero_and_keeps_the_client_address() {
    ...
    assert_eq!(
        detection.timestamp_shape,
        Some(looqlog_core::TimestampShape::Clf)
    );
...
fn slash_date_fixture_is_dated_without_inferring_a_year() {
    ...
    assert_eq!(
        ...
        Some(looqlog_core::TimestampShape::SlashDate)
    );
    assert!(entries.iter().all(|e| e.timestamp.is_some()));
...
fn epoch_fixture_is_dated_from_a_bare_leading_integer() {
    ...
    assert_eq!(
        parser.detection().unwrap().timestamp_shape,
        Some(looqlog_core::TimestampShape::Epoch)
    );
...
fn bugreport_shaped_input_parses_its_logcat_lines_with_no_sticky_hint() {
    ...
    assert_eq!(detection.outcome, looqlog_core::DetectionOutcome::Fallback);
