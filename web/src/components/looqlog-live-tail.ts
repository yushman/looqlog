...
// the active time range — file mode's is `looqlog-app.ts`, stream mode's is this
// component): it owns `activeRange` and passes it to the timeline and table
// exactly like `looqlog-app.ts` does, rather than wiring them to each other.
//
// Virtual-scrolled via `<looqlog-entry-table>` (`entry-table` spec) — this used to
...
export class LooqlogLiveTail extends HTMLElement {
  ...
  // entries"): same predicate model as `looqlog-app.ts`'s file-mode shell, this
  ...
  /** Same mirroring as `looqlog-app.ts`: the table owns the drag, this shell owns
  ...
  connectedCallback(): void {
    ...
    // The same `looqlog-workspace` file mode mounts (design.md D1): only the contents
      ...
      <details class="rail-section">
        ...
        <div class="rail-section-body">
          <p class="privacy-note">
            ...
            <code>looqlog</code> process and this browser — a different guarantee from
    ...
    `;
    ws.pane("table").innerHTML = `<looqlog-entry-table></looqlog-entry-table>`;
  ...
  /** The one place `activeRange` is written (D7), same pattern as `looqlog-app.ts`. */
  ...
  /** Same reasoning as `looqlog-app.ts`'s method of the same name (D2): the filter
  ...
  /** Same caveat, same "flush before copy" reasoning as `looqlog-app.ts`'s
...
customElements.define("looqlog-live-tail", LooqlogLiveTail);
