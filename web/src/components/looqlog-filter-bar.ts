...
import { isNarrowLayout } from "./looqlog-workspace";

/** UI-side cap on how many values a field lists, independent of the parser's own
 * cardinality cap (`crates/looqlog-core/src/fields.rs`, `DEFAULT_FIELD_VALUE_CAP` =
...
export class LooqlogFilterBar extends HTMLElement {
   ...
   * reported yet in a fresh live session — but the caller (`looqlog-app.ts`) checks
  ...
  private scheduleEmit(): void {
    ...
    this.searchDebounceTimer = setTimeout(() => {
      ...
      this.emitChange();
    }, LooqlogFilterBar.SEARCH_DEBOUNCE_MS);
  }
...
customElements.define("looqlog-filter-bar", LooqlogFilterBar);
